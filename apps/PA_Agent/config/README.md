...
| `primary_provider_enabled` | bool | `true` | 是否启用主线路 |
| `retry_primary_each_analysis` | bool | `false` | 主线路曾在本次运行中失败后，每次新分析是否重新先尝试主线路；失败后再切换备用线路 |
| `backup_provider_enabled` | bool | `false` | 是否启用备用线路 |
