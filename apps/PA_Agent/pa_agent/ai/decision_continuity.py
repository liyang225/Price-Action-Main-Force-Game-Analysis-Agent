...
def _parse_local_iso_ms(iso: str | None) -> int | None:
    ...
    try:
        ...
        # Naive PA timestamps are local wall-clock values.  Use a fixed
        # timezone so elapsed bars do not depend on the host machine locale.
...
def assess_combined_plan_invalidation(
    ...
    *,
    ...
    # A resting order cannot hit its stop before it has filled. Evaluate the
    # stop-loss only for market plans or after a limit/breakout entry touched.
    order_type = str((prev_decision or {}).get("order_type") or "").strip()
    if (
        order_type in ("限价单", "突破单")
        and not limit_triggered
        and prev_stage1 is not None
    ):
        invalidated, invalidation_reason = False, ""
    else:
...
def build_continuity_context(
    *,
    ...
    if prev_decision is None and allow_trade_csv_fallback:
...
def audit_relation_fields(
    ...
    *,
    ...
    prev_sign = order_direction_sign(str(prev_decision.get("order_direction") or ""))
    curr_sign = order_direction_sign(str((curr_decision or {}).get("order_direction") or ""))
    if prev_sign and curr_sign and prev_sign != curr_sign:
        # Relation and validity are separate audit dimensions.  Preserve the
        # explicit flip label even when the previous resting plan also expired.
        rel_key = _REL_FLIP
    if (
        rel_key == _REL_FLIP
