...
def _normalize_stage2_enum_aliases(out: dict[str, Any]) -> bool:
    ...
    if isinstance(decision, dict):
        ...
        if mapped_dir and mapped_dir != raw_dir:
            ...
            changed = True
        # The schema requires TP2 for a trade.  Older PA responses only
        # supplied TP1; derive a conservative second target so normalization
        # can validate the plan instead of discarding its entry fields.
        if (
            decision.get("order_type") in _TRADE_ORDER_TYPES
            and decision.get("take_profit_price_2") is None
        ):
            try:
                entry = float(decision.get("entry_price"))
                tp1 = float(decision.get("take_profit_price"))
                if tp1 > entry:
                    decision["take_profit_price_2"] = tp1 + (tp1 - entry)
                elif tp1 < entry:
                    decision["take_profit_price_2"] = tp1 - (entry - tp1)
                else:
                    decision["take_profit_price_2"] = tp1
                changed = True
    ...
    if isinstance(bar_analysis, dict):
        ...
        if isinstance(entry_bar, dict):
            raw_strength = entry_bar.get("strength")
            if str(raw_strength or "").strip().lower() == "pending":
                entry_bar["strength"] = "not_triggered"
                changed = True
            raw_freshness = str(entry_bar.get("freshness") or "").strip().lower()
            if raw_freshness == "limit_order_pending":
...
def _coerce_decision_when_trade_metrics_fail(
    ...
    *,
    ...
    if isinstance(prediction, dict):
        next_dir = str(prediction.get("direction") or "").strip().lower()
        order_dir = str(decision.get("order_direction") or "").strip().lower()
        if (next_dir == "bullish" and ("空" in order_dir or order_dir == "short")) or (
            next_dir == "bearish" and ("多" in order_dir or order_dir == "long")
        ):
            # The explicit cycle-direction guard owns this rejection and its
            # auditable wait outcome; do not preempt it as a metric failure.
            return False
...
def _enforce_next_cycle_direction_guard(out: dict[str, Any]) -> None:
    """Keep the terminal state aligned with the hard direction guard."""
    prediction = out.get("next_cycle_prediction")
    decision = out.get("decision")
    ...
    next_dir = str(prediction.get("direction") or "").strip().lower()
    order_dir = str(decision.get("order_direction") or "").strip().lower()
    blocked = (next_dir == "bullish" and ("空" in order_dir or order_dir in {"short", "sell"})) or (
        next_dir == "bearish" and ("多" in order_dir or order_dir in {"long", "buy"})
    )
    if blocked:
...
def normalize_stage2(
    ...
    *,
    ...
    # Decision-node routing may run after the initial price normalization.
    # Re-apply the canonical basis rule at the final order boundary.
    decision = out.get("decision")
    if isinstance(decision, dict):
        normalize_breakout_entry_price(decision, kline_frame=kline_frame)
    ...
    _enforce_next_cycle_direction_guard(out)
