...
def apply_workbuddy_provider_to_settings(
    ...
    *,
    ...
    """Populate *settings.provider* from the local WorkBuddy environment.

    This function deliberately performs no network probe.  Connectivity is
    validated by the real request path, avoiding a duplicate streaming request
    during startup, settings application, or analysis preparation.

    Returns None on success, or a user-facing local-configuration error string.
    """
    from pa_agent.ai.cursor_connector import is_openclaw_cs_model
    from pa_agent.ai.qclaw_connector import is_openclaw_model
