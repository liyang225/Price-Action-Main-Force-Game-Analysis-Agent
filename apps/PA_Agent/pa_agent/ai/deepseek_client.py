...
from pa_agent.config.settings import AIProviderSettings, normalize_provider_base_url
...
def _is_aliyun_maas(base_url: str) -> bool:
    """Alibaba Cloud MaaS (百炼) OpenAI-compatible gateway."""
    return "maas.aliyuncs.com" in (base_url or "").lower()


def _is_sensenova(base_url: str) -> bool:
    """SenseNova's OpenAI-compatible gateway uses its own thinking enum."""
...
# Alibaba Cloud MaaS (百炼): max_tokens must be in [1, 65536].
_ALIYUN_MAAS_MAX_OUTPUT_TOKENS = 65_536
...
def _thinking_enabled(extra_body: dict[str, Any], effort: str | None) -> bool:
    if extra_body:
        if extra_body.get("chat_template_kwargs", {}).get("enable_thinking"):
            return True
        return extra_body.get("thinking", {}).get("type") in (
            "enabled",
            "adaptive",
            "auto",
        )
...
def _configured_max_output_tokens(settings: AIProviderSettings) -> int | None:
    overrides = getattr(settings, "max_tokens_by_base_url", {}) or {}
    key = normalize_provider_base_url(settings.base_url)
...
def _provider_max_output_tokens(settings: AIProviderSettings) -> int:
    ...
    configured = _configured_max_output_tokens(settings)
    ...
    if _is_aliyun_maas(settings.base_url):
        return _ALIYUN_MAAS_MAX_OUTPUT_TOKENS
    if _is_mimo(settings):
...
def _resolve_thinking_params(
    ...
    *,
    ...
    if _is_sensenova(settings.base_url):
        # SenseNova accepts enabled/disabled/auto, but rejects the newer
        # DeepSeek ``adaptive`` dialect even when the model id contains
        # ``deepseek``.  Keep PA's boolean setting truthful: on means enabled.
        return {"thinking": {"type": "enabled" if _thinking else "disabled"}}, None
