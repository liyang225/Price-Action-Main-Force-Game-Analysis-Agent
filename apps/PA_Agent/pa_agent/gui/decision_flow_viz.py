...
# Palette-mapped accents (was: neon HUD colors) — values track theme tokens.
_NEON_CYAN = T.ACCENT
_NEON_BLUE = T.ACCENT_HOVER
_NEON_VIOLET = T.CHART_LINE_2
_NEON_AMBER = T.WARNING
_GLASS_BG = T.SURFACE_2
...
class _FlowScene(QGraphicsScene):
    def drawBackground(self, painter: QPainter, rect: QRectF) -> None:  # noqa: N802
        # Flat terminal base — no gradients, no glow, no scan lines.
        painter.fillRect(rect, QBrush(QColor(T.BG)))
        ...
        painter.setPen(QPen(QColor(255, 255, 255, 13)))
        x = left
        while x < rect.right():
            painter.drawLine(int(x), int(rect.top()), int(x), int(rect.bottom()))
            x += major
...
class _BranchEdge(QGraphicsObject):
    ...
    def paint(self, painter: QPainter, _option: Any, _widget: Any = None) -> None:
        ...
        # Flat connector: weight and dash carry state, no glow underlay.
        pen = QPen(col)
        pen.setWidth(2 if self._active else 1)
        if not self._active:
...
class _DecisionNode(QGraphicsObject):
    ...
    def paint(self, painter: QPainter, _option: Any, _widget: Any = None) -> None:
        ...
        # Flat card; the border steps up on hover instead of a glow halo.
        painter.setBrush(QBrush(QColor(T.SURFACE_2)))
        border_alpha = 220 if self._hover else 120
        painter.setPen(QPen(QColor(accent.red(), accent.green(), accent.blue(), border_alpha), 1))
        painter.drawRoundedRect(rect, 6, 6)

        # A flat 3px status bar makes the answer visible even when zoomed out.
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(QBrush(accent))
        painter.drawRoundedRect(QRectF(-w / 2, 0, 3, h), 1, 1)
...
class _TerminalNode(QGraphicsObject):
    ...
    def paint(self, painter: QPainter, _option: Any, _widget: Any = None) -> None:
        ...
        # Flat card; a 3px top rule in the outcome colour replaces the halo.
        painter.setBrush(QBrush(QColor(T.SURFACE_2)))
        ...
        painter.drawRoundedRect(QRectF(-w / 2, 0, w, 3), 1, 1)

        painter.setFont(_font_ui(10))
        painter.setPen(QPen(QColor(T.TEXT_MUTED)))
        painter.drawText(
            QRectF(-w / 2, 14, w, 18),
            int(Qt.AlignmentFlag.AlignCenter),
        ...
        painter.setFont(_font_ui(15, bold=True))
...
class DecisionFlowVizPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None, *, show_controls: bool = True) -> None:
        ...
        # Ambient neon animation was removed with the flat redesign; the FX
        # timer stays constructed (legacy references) but is never started.
    ...
    def show_insufficient_data(self, record: Any) -> None:
        ...
        self._fit_scene(rect)
        self._hud_label.setText(
            f"<span style='color:{T.WARNING}'>数据不足：{check_zh}</span>"
        )
