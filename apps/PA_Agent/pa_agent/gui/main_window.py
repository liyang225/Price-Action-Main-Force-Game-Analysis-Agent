...
from PyQt6.QtGui import QAction, QColor, QCloseEvent, QPainter, QPen, QResizeEvent, QShowEvent
...
from pa_agent.gui.widgets.split_hotzone import SplitHotzone

logger = logging.getLogger(__name__)

# Zombie timeout in milliseconds (5 seconds)
_WORKER_JOIN_TIMEOUT_MS = 5000
_FUTU_SYMBOL_FORMAT_HELP = FUTU_SYMBOL_FORMAT_HELP
_SESSION_SPLIT_HOTZONE_RATIOS: dict[str, float] = {}


class _ElidedLabel(QLabel):
    """Label that keeps the complete title available while eliding overflow."""
    ...
    def _refresh_display_text(self) -> None:
        available_width = max(0, self.width() - self._HORIZONTAL_PADDING)
...
class _TimeframeComboBox(QComboBox):
    """Period selector with a compact up/down disclosure marker."""

    def paintEvent(self, event) -> None:  # noqa: N802
        super().paintEvent(event)
        painter = QPainter(self)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        painter.setPen(QPen(QColor("#9AA5B1"), 1.4))
        right = self.width() - 10
        center_y = self.height() // 2
        painter.drawLine(right - 4, center_y - 2, right, center_y - 6)
        painter.drawLine(right, center_y - 6, right + 4, center_y - 2)
        painter.drawLine(right - 4, center_y + 2, right, center_y + 6)
        painter.drawLine(right, center_y + 6, right + 4, center_y + 2)
...
def _tradingview_exchange_from_context(ctx: AppContext) -> str:
    settings = getattr(ctx, "settings", None)
    general = getattr(settings, "general", None)
    return str(getattr(general, "last_tradingview_exchange", "") or "").strip()
...
class MainWindow(QMainWindow):
    ...
    workspace_continuous_tracking_changed = pyqtSignal(bool)
    order_opportunity_changed = pyqtSignal(bool)
    order_opportunity_detected = pyqtSignal(dict)
    ...
    def __init__(
        ...
        *,
        ...
        self._target_name = str(target_name or "").strip()
        if embedded:
            self.setAttribute(Qt.WidgetAttribute.WA_DontShowOnScreen, True)
            self.setParent(parent, Qt.WindowType.Widget)
            self.setAttribute(Qt.WidgetAttribute.WA_DontCreateNativeAncestors, True)
        self.setWindowTitle("PA Agent — Trading Terminal")
        # Embedded terminals are inserted directly into the workspace tab.
        # Giving one a top-level default size before it has a tab parent can
        # create a transient native window surface on Windows.
        if not embedded:
        ...
        self._managed_order_opportunity = False
        self._switching = False
        self._chart_refresh_paused = False
        self._chart_dragging = False
        self._deferred_chart_bars: list[Any] | None = None
        ...
        self._history_overlay_active = False
        self._history_record_path: str | None = None
        ...
        if self._embedded_mode:
            self.menuBar().hide()
            self._suspend_initial_embedded_scrollbars()
    ...
    def _setup_ui(self) -> None:
        ...
        self._history_panel = self._ai_sidebar.history
        self._history_panel.record_selected.connect(self._on_history_record_selected)
        self._ai_sidebar.previous_history_requested.connect(lambda: self._navigate_history(1))
        self._ai_sidebar.next_history_requested.connect(lambda: self._navigate_history(-1))
        self._ai_sidebar.history_navigation_requested.connect(self._navigate_history)
        ...
        self._demo_mode_label.setStyleSheet(
            f"color: {T.WARNING}; font-weight: 600; padding-left: 4px;"
        )
    ...
    def _embedded_breadcrumb_text(self, symbol: str) -> str:
        if self._target_name:
            return self._target_name
        normalized = str(symbol or "").strip()
        if normalized.upper() in {"159732", "SZ.159732", "159732.SZ"}:
            return "消费电子ETF (159732.SZ )"
        return normalized or "未选择标的"

    def _set_embedded_breadcrumb_text(self, text: str) -> None:
        """Show the complete embedded target name without a fixed-width crop."""
        self._instrument_breadcrumb.setText(text)
        self._embedded_breadcrumb_full_text = str(text)
        self._update_embedded_breadcrumb_width()

    def _update_embedded_breadcrumb_width(self) -> None:
        """Fit the breadcrumb to its text until the toolbar runs out of room."""
        breadcrumb = getattr(self, "_instrument_breadcrumb", None)
        group = getattr(self, "_instrument_control_group", None)
        if breadcrumb is None or group is None:
            return
        text = str(getattr(self, "_embedded_breadcrumb_full_text", "") or "")
        content_width = breadcrumb.fontMetrics().horizontalAdvance(text) + 24
        parent = group.parentWidget()
        available_width = 0 if parent is None else parent.width()
        if available_width > 0:
            controls = (
                getattr(self, "_fetch_data_btn", None),
                getattr(self, "_submit_btn", None),
                getattr(self, "_wait_close_checkbox", None),
                getattr(self, "_wait_close_countdown_label", None),
                getattr(self, "_keep_analysis_checkbox", None),
                getattr(self, "_decision_badge", None),
                getattr(self, "_flow_bar", None),
            )
            fixed_controls_width = sum(
            ...
            group_overhead = max(
                0,
                group.sizeHint().width() - breadcrumb.width(),
            )
            spacing = getattr(getattr(self, "_control_layout", None), "spacing", lambda: 12)()
            # Seven visible toolbar items create six standard gaps; include the
            # fixed data/action separation and a small safety margin.
            reserved = fixed_controls_width + group_overhead + spacing * 6 + 32 + 24
            max_content_width = max(96, available_width - reserved)
            target_width = min(content_width, max_content_width)
        else:
            target_width = content_width
        breadcrumb.setFixedWidth(max(96, int(target_width)))
    ...
    def _build_workbench(self) -> QWidget:
        ...
        # The terminal content sits clear of the embedded workspace tab strip
        # and status dock, while leaving the chart as the only compressed area.
        outer_layout.setContentsMargins(0, 0, 0, 6)
        # Preserve a one-pixel breathing space between the control-bar
        # separator and the chart container below it.
        ...
        control_section_layout.setSpacing(3)

        control_top_separator = QFrame()
        control_top_separator.setObjectName("controlBarSeparator")
        control_top_separator.setFrameShape(QFrame.Shape.HLine)
        control_top_separator.setFixedHeight(1)
        control_section_layout.addWidget(control_top_separator)

        self._technical_controls = QWidget()
        ctrl_layout = QHBoxLayout(self._technical_controls)
        ctrl_layout.setContentsMargins(12, 0, 12, 0)
        ctrl_layout.setSpacing(12)
        self._control_layout = ctrl_layout

        self._instrument_breadcrumb = _ElidedLabel()
        self._instrument_breadcrumb.setObjectName("instrumentBreadcrumb")
        self._instrument_source_tag = QLabel()
        self._instrument_source_tag.setObjectName("sourceTag")
        self._instrument_control_group = QFrame()
        self._instrument_control_group.setObjectName("instrumentControlGroup")
        instrument_layout = QHBoxLayout(self._instrument_control_group)
        instrument_layout.setContentsMargins(0, 0, 0, 0)
        instrument_layout.setSpacing(0)
        instrument_layout.addWidget(self._instrument_source_tag)
        source_separator = QFrame()
        source_separator.setObjectName("instrumentControlSeparator")
        source_separator.setFrameShape(QFrame.Shape.VLine)
        instrument_layout.addWidget(source_separator)
        instrument_layout.addWidget(self._instrument_breadcrumb)
        # Pipeline flow bar (5-step) is hosted in the strategy tab row, docked
        # right; the action cluster sits in the selector row below.
        from pa_agent.gui.widgets.flow_bar import FlowBar

        self._flow_bar = FlowBar()
        self._flow_bar.setFixedWidth(280)
        self._module_controls = QWidget()
        self._module_controls_stack = QStackedLayout(self._module_controls)
        self._module_controls_stack.setContentsMargins(0, 0, 0, 0)
        self._module_controls_stack.addWidget(self._technical_controls)
        self._pipeline_host = QWidget()
        self._pipeline_stack = QStackedLayout(self._pipeline_host)
        self._pipeline_stack.setContentsMargins(0, 0, 0, 0)
        self._pipeline_stack.addWidget(self._flow_bar)

        # Strategy modules live beside the instrument identity and switch the
        # product workspace below without leaving the current instrument page.
        strategy_section = QWidget()
        strategy_section_layout = QVBoxLayout(strategy_section)
        strategy_section_layout.setContentsMargins(0, 0, 0, 1)
        strategy_section_layout.setSpacing(3)
        strategy_row = QHBoxLayout()
        strategy_row.setContentsMargins(12, 0, 12, 0)
        strategy_row.setSpacing(10)
        strategy_row.addWidget(self._instrument_control_group)
        self._strategy_browser = QTabBar()
        self._strategy_browser.setObjectName("strategyBrowser")
        self._strategy_browser.setExpanding(False)
        self._strategy_browser.setDrawBase(False)
        self._strategy_browser.addTab("技术分析")
        self._strategy_browser.addTab("二阶博弈")
        self._strategy_browser.currentChanged.connect(self._on_strategy_module_selected)
        strategy_row.addWidget(self._strategy_browser)
        strategy_row.addStretch(1)
        strategy_row.addWidget(self._pipeline_host)
        strategy_section_layout.addLayout(strategy_row)

        # Keep the module row visually separate from the action controls.  The
        # divider sits 3 px below this row; the complete inter-row gap is 8 px.
        strategy_row_separator = QFrame()
        strategy_row_separator.setObjectName("strategyRowSeparator")
        strategy_row_separator.setFrameShape(QFrame.Shape.HLine)
        strategy_row_separator.setFixedHeight(1)
        strategy_section_layout.addWidget(strategy_row_separator)
        control_section_layout.addWidget(strategy_section)
        ...
        ctrl_layout.addWidget(self._data_source_label)
        ...
        # Embedded watchlist terminals carry their own exchange in this context.
        saved_ex = _tradingview_exchange_from_context(self._ctx)
        ...
        self._futu_symbol_help_button = QToolButton()
        self._futu_symbol_help_button.setObjectName("futuSymbolFormatHelp")
        self._futu_symbol_help_button.setText("?")
        self._futu_symbol_help_button.setFixedSize(20, 20)
        self._futu_symbol_help_button.setToolTip(_FUTU_SYMBOL_FORMAT_HELP)
        self._futu_symbol_help_button.clicked.connect(self._show_futu_symbol_format_help)
        ctrl_layout.addWidget(self._futu_symbol_help_button)
        self._apply_data_source_symbol_placeholder()
        self._populate_symbol_combo_for_source()

        self._symbol_alert_label = QLabel("")
        self._symbol_alert_label.setStyleSheet(f"color: {T.DANGER}; font-size: 11px;")
        self._symbol_alert_label.setWordWrap(True)
        self._symbol_alert_label.hide()
        ctrl_layout.addWidget(self._symbol_alert_label)

        # Timeframe
        self._timeframe_label = QLabel("周期:")
        ctrl_layout.addWidget(self._timeframe_label)
        self._tf_combo = _TimeframeComboBox()
        self._tf_combo.setObjectName("timeframeCombo")
        self._tf_combo.addItems(["1m", "5m", "15m", "30m", "1h", "4h", "1d"])
        self._tf_combo.setCurrentText(_last_tf)
        self._tf_combo.setFixedWidth(72)
        if self._embedded_mode:
            timeframe_separator = QFrame()
            timeframe_separator.setObjectName("instrumentControlSeparator")
            timeframe_separator.setFrameShape(QFrame.Shape.VLine)
            instrument_layout.addWidget(timeframe_separator)
            instrument_layout.addWidget(self._tf_combo)
        else:
            ctrl_layout.addWidget(self._tf_combo)
        self._populate_timeframe_combo_for_source(_last_tf)
        self._sync_tv_exchange_visibility()

        self._embedded_metadata_widgets = (
            self._data_source_label,
            self._data_source_combo,
            self._tv_exchange_label,
            self._tv_exchange_combo,
            self._variety_label,
            self._variety_combo,
            self._symbol_label,
            self._symbol_combo,
            self._futu_symbol_help_button,
            self._timeframe_label,
            self._symbol_alert_label,
        )
        if self._embedded_mode:
            for widget in self._embedded_metadata_widgets:
                widget.hide()
            self._set_embedded_breadcrumb_text(self._embedded_breadcrumb_text(_last_symbol))
            self._instrument_source_tag.setText("Futu")

        # Action cluster for the 技术分析 module, docked right in the selector
        # row below the strategy tabs.  Selection controls remain in ctrl_layout.
        self._action_controls_host = QWidget()
        self._action_controls_host.setObjectName("strategyActionHost")
        self._action_controls_stack = QStackedLayout(self._action_controls_host)
        self._action_controls_stack.setContentsMargins(0, 0, 0, 0)

        self._technical_action_bar = QWidget()
        self._technical_action_bar.setObjectName("technicalActionBar")
        action_layout = QHBoxLayout(self._technical_action_bar)
        action_layout.setContentsMargins(0, 0, 0, 0)
        action_layout.setSpacing(12)

        self._fetch_data_btn = QPushButton("获取数据")
        self._fetch_data_btn.setObjectName("fetchDataButton")
        self._fetch_data_btn.setMinimumWidth(100)
        ...
        self._submit_btn.setObjectName("accentButton")
        ...
        self._wait_close_checkbox = QCheckBox("最新K线收盘后再分析")
        ...
        self._wait_close_checkbox.setToolTip(
            "勾选后，提交分析会先等待当前未收盘K线走完，再抓取数据并开始分析"
        )
        ...
        # 演示模式按钮已移至左上角「设置」菜单，此处保留引用供内部逻辑使用（不加入 action_layout）
        ...
        self._keep_analysis_checkbox = ShimmerButton()
        self._keep_analysis_checkbox.setObjectName("continuousTrackingButton")
        self._keep_analysis_checkbox.set_tracking_labels(
            active="持续分析",
            inactive="持续分析",
        )
        ...
        self._action_controls_stack.addWidget(self._technical_action_bar)
        ...
        shared_control_row = QHBoxLayout()
        shared_control_row.setContentsMargins(0, 0, 12, 0)
        shared_control_row.setSpacing(0)
        shared_control_row.addWidget(self._module_controls, 1)
        shared_control_row.addWidget(self._action_controls_host)
        control_section_layout.addLayout(shared_control_row)

        control_bottom_separator = QFrame()
        control_bottom_separator.setObjectName("controlBarSeparator")
        control_bottom_separator.setFrameShape(QFrame.Shape.HLine)
        control_bottom_separator.setFixedHeight(1)
        control_section_layout.addWidget(control_bottom_separator)
        outer_layout.addWidget(control_section)

        content_section = QWidget(tab)
        ...
        content_layout.addWidget(self._api_key_alert_label)

        status_row = QHBoxLayout()
        status_row.addStretch()
        self._last_refresh_ts: float = 0.0
        self._analysis_count_label = QLabel("分析次数: 0")
        self._analysis_count_label.setObjectName("mutedLabel")
        status_row.addWidget(self._analysis_count_label)
        ...
        workbench = QSplitter(Qt.Orientation.Horizontal, content_section)
        self._workbench = workbench

        chart_column = QWidget(workbench)
        self._chart_widget = ChartWidget(chart_column)
        self._chart_widget.chart_dragging_changed.connect(
            self._on_chart_dragging_changed
        )
        chart_column.setSizePolicy(
            QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Expanding
        )
        chart_column_layout = QVBoxLayout(chart_column)
        ...
        from pa_agent.gui.widgets.indicator_panel import (
            IndicatorPanel,
            MovingAverageToolbar,
            _INDICATOR_PANEL_DEFAULT_HEIGHT_PX,
        )

        self._market_summary_strip = MarketSummaryStrip(chart_column)
        self._chart_widget.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        self._apply_chart_display_settings()
        self._market_summary_strip.setFixedHeight(44)
        chart_column_layout.addWidget(self._market_summary_strip)
        self._moving_average_toolbar = MovingAverageToolbar(chart_column)
        self._moving_average_toolbar.overlay_changed.connect(
            self._chart_widget.set_moving_average_overlays
        )
        self._moving_average_toolbar.seq_labels_changed.connect(
            self._chart_widget.set_seq_labels_enabled
        )
        chart_column_layout.addWidget(self._moving_average_toolbar)
        self._chart_indicator_splitter = QSplitter(Qt.Orientation.Vertical, chart_column)
        self._chart_indicator_splitter.setObjectName("chartIndicatorSplitter")
        self._chart_indicator_splitter.setChildrenCollapsible(False)
        self._chart_indicator_splitter.setHandleWidth(6)
        self._chart_indicator_splitter.setOpaqueResize(False)
        self._chart_indicator_splitter.addWidget(self._chart_widget)
        self._indicator_panel = IndicatorPanel(self._chart_widget, chart_column)
        self._chart_widget.frame_rendered.connect(self._indicator_panel.set_frame)
        self._chart_indicator_splitter.addWidget(self._indicator_panel)
        self._chart_indicator_hotzone = self._chart_indicator_splitter.handle(1)
        self._chart_indicator_hotzone_controller = SplitHotzone(
            self._chart_indicator_hotzone
        )
        self._chart_indicator_hotzone_controller.ratioChanged.connect(
            self._persist_chart_indicator_split_ratio
        )
        self._chart_indicator_splitter.setStretchFactor(0, 1)
        self._chart_indicator_splitter.setStretchFactor(1, 0)
        self._indicator_panel_default_height_px = _INDICATOR_PANEL_DEFAULT_HEIGHT_PX
        self._indicator_panel_default_height_applied = False
        chart_column_layout.addWidget(self._chart_indicator_splitter, stretch=1)
        workbench.addWidget(chart_column)
        ...
        # Give the analysis workspace a little more room than the chart.
        # These sizes are the fallback until a saved user ratio is restored.
        workbench.setStretchFactor(0, 45)
        workbench.setStretchFactor(1, 55)
        workbench.setHandleWidth(18)
        workbench.setSizes([450, 550])
        self._workbench_split_ratio_applied = False
        workbench.splitterMoved.connect(
            lambda _position, _index: self._persist_workbench_split_ratio()
        )

        # ── SummaryStrip: 5-metric card strip above workbench ─────────────────
        from pa_agent.gui.widgets.summary_strip import SummaryStrip
        self._summary_strip = SummaryStrip(tab)
        ...
        self._second_order_flow_bar = FlowBar()
        self._second_order_flow_bar.setFixedWidth(280)
        self._second_order_flow_bar.set_step_names(
            ("行情", "材料", "推演", "闸门", "归档")
        )
    ...
    def _second_order_handoff_payload(
        ...
        return {
            ...
            "settlement_mode": self._second_order_settlement_mode(),
            "data_source": self._current_data_source_kind(),
            "stage1_diagnosis": stage1 if isinstance(stage1, dict) else None,
    ...
    def _second_order_settlement_mode(self) -> str:
        ...
        if store is None:
            return "unset"
        mode = store.mode_for(symbol)
        if mode != "unset":
            return mode
        digits = "".join(character for character in symbol if character.isdigit())
        return store.mode_for(digits) if len(digits) == 6 else "unset"

    def _sync_second_order_after_stage2(
        self,
        ...
        *,
        ...
        if auto_run:
            ...
            self._strategy_browser.setCurrentIndex(1)
            workspace.set_pa_payload(payload)
            workspace.run_automatic_analysis()
            self._status_bar.showMessage("PA 阶段二完成，正在自动运行 T+1 二阶推演")
        else:
            workspace.set_pa_payload(payload)

    def _on_strategy_module_selected(self, index: int) -> None:
        if not hasattr(self, "_strategy_stack"):
            return
        module_index = 1 if index == 1 else 0
        self._strategy_stack.setCurrentIndex(module_index)
        self._module_controls_stack.setCurrentIndex(module_index)
    ...
    def _on_second_order_pipeline_step_changed(self, index: int, status: str) -> None:
        flow = getattr(self, "_second_order_flow_bar", None)
        if flow is not None:
            flow.set_step_status(index, status)
    ...
    def _sync_tv_exchange_visibility(self) -> None:
        """Show the venue selector for sources that accept SSE/SZSE input."""
        visible = (
            self._current_data_source_kind() in ("tradingview", "futu")
            and not getattr(self, "_demo_mode", False)
        )
        for w in (
    ...
    def _apply_tv_exchange_to_source(self, data_source: Any) -> None:
        ...
        elif isinstance(data_source, FutuSource):
            data_source.set_exchange(self._tv_exchange_text())
    ...
    def _persist_tradingview_exchange(self) -> None:
        settings = getattr(self._ctx, "settings", None)
        if settings is None or self._embedded_mode:
            return
    ...
    def _apply_data_source_symbol_placeholder(self) -> None:
        ...
        if line is None:
            return
        kind = self._current_data_source_kind()
        help_button = getattr(self, "_futu_symbol_help_button", None)
        if help_button is not None:
            help_button.setVisible(kind == "futu")
        ...
        if kind == "tradingview":
            ...
            line.setPlaceholderText("富途代码：SH.600519、HK.00700 或 US.AAPL；A股可输入 600519")
        else:
            line.setPlaceholderText("输入 MT5 品种名，如 XAUUSDm…")

    def _show_futu_symbol_format_help(self) -> None:
        button = self._futu_symbol_help_button
        QToolTip.showText(
            button.mapToGlobal(button.rect().bottomLeft()),
            _FUTU_SYMBOL_FORMAT_HELP,
            button,
        )
    ...
    def _populate_timeframe_combo_for_source(self, preferred_text: str | None = None) -> None:
        data_source = getattr(self._ctx, "data_source", None)
    ...
    def _switch_data_source(self, kind: str) -> None:
        ...
        try:
            ...
            # Restore the selected venue before applying it to the data source.
            if kind in ("tradingview", "futu"):
                settings = getattr(self._ctx, "settings", None)
                saved_ex = ""
                if settings is not None:
    ...
    def _on_symbol_combo_text_changed(self, _text: str = "") -> None:
        ...
        if self._embedded_mode:
            self._set_embedded_breadcrumb_text(self._embedded_breadcrumb_text(_text))
        self._update_symbol_data_alert()
    ...
    def _update_symbol_data_alert(self) -> None:
        ...
        if kind == "tradingview":
            if symbol.lower().endswith("m") and len(symbol) > 2:
                ...
                label.setStyleSheet(f"color: {T.WARNING}; font-size: 11px;")
                label.show()
                return
            label.hide()
            return
    ...
    def _current_history_symbol(self) -> str:
        combo = getattr(self, "_symbol_combo", None)
        if combo is not None:
            return combo.currentText().strip()
        settings = getattr(self._ctx, "settings", None)
        if settings is not None:
            return str(getattr(settings.general, "last_symbol", "") or "").strip()
        return ""

    def _refresh_history_ui(self) -> None:
        symbol = self._current_history_symbol()
        history = getattr(self, "_history_panel", None)
        if history is not None:
            history.set_symbol(symbol)
        label = getattr(self, "_analysis_count_label", None)
        if label is None:
            return
        try:
            from pa_agent.records.history_analysis import count_history_records

            count = count_history_records(symbol) if symbol else 0
        except Exception as exc:  # noqa: BLE001
            logger.debug("History count refresh failed: %s", exc)
            count = 0
        label.setText(f"分析次数: {count}")

    def _archive_completed_history_record(self, record: Any) -> None:
        if getattr(self, "_demo_mode", False) or getattr(self, "_history_overlay_active", False):
            return
        if getattr(record, "exception", None) is not None:
            return
        if not getattr(record, "stage2_decision", None):
            return
        try:
            from pa_agent.records.history_analysis import save_history_record

            save_history_record(record)
            self._refresh_history_ui()
        except Exception as exc:  # noqa: BLE001
            logger.warning("Failed to save history analysis record: %s", exc)

    def _clear_history_overlay(self) -> None:
        panel = getattr(self, "_history_panel", None)
        if panel is not None:
            panel.clear_selection()
        if not getattr(self, "_history_overlay_active", False):
            return
        self._history_overlay_active = False
        self._history_record_path = None
        self._ai_sidebar.set_history_navigation_enabled(False, False)
        self._status_bar.showMessage("已退出历史记录回看")

    def _prepare_history_record_view(self, record: Any, path_name: str) -> bool:
        try:
            ...
            QMessageBox.warning(self, "历史记录", f"无法加载历史分析记录：\n{path_name}\n{exc}")
            return False
        ...
        self._analysis_previous_record = None
        self._last_frame_ready_bars = None
        ...
        if debug is not None:
            debug.clear()
        if getattr(self, "_decision_panel", None) is not None:
            self._decision_panel.clear()
        if getattr(self, "_future_trend_panel", None) is not None:
            self._future_trend_panel.clear()
        if getattr(self, "_decision_tree_panel", None) is not None:
    ...
    def _on_history_record_selected(self, path_str: str, *, focus_stream: bool = True) -> None:
        from pathlib import Path

        from pa_agent.records.history_analysis import load_history_record

        path = Path(path_str)
        record = load_history_record(path)
        if record is None:
            QMessageBox.warning(self, "历史记录", f"无法读取历史分析记录：\n{path}")
            return
        if not self._prepare_history_record_view(record, path.name):
            return

        self._history_overlay_active = True
        self._history_record_path = str(path)
        try:
            self._on_record_ready_impl(record)
            self._on_analysis_finished(record.stage2_decision or {})
        finally:
            self._refresh_history_ui()
        self._update_history_navigation()
        if focus_stream:
            self._ai_sidebar.focus_stream()
        self._status_bar.showMessage(f"已载入历史分析记录：{path.name}")

    def _update_history_navigation(self) -> None:
        current = str(getattr(self, "_history_record_path", "") or "")
        entries = getattr(self._history_panel, "_entries", [])
        ...
        try:
            ...
            self._ai_sidebar.set_history_navigation_enabled(False, False)
            return
        self._ai_sidebar.set_history_navigation_enabled(index + 1 < len(paths), index > 0)
    ...
    def _navigate_history(self, offset: int) -> None:
        ...
        paths = [str(entry.path) for entry in entries]
        try:
            target_index = paths.index(current) + offset
        except ValueError:
            return
        if 0 <= target_index < len(paths):
            self._on_history_record_selected(paths[target_index], focus_stream=False)
    ...
    def _set_chart_refresh_paused(self, paused: bool) -> None:
        ...
        switch = getattr(self, "_chart_refresh_switch", None)
        if switch is None:
            switch = getattr(self, "_resume_chart_btn", None)
        if switch is not None and switch.isChecked() == paused:
            # Internal state changes must not re-enter the user toggle handler.
            switch.blockSignals(True)
            switch.setChecked(not paused)
            switch.blockSignals(False)

    def _on_chart_refresh_switch_changed(self, _state: int) -> None:
        """Apply the user's live-chart switch choice."""
        switch = getattr(self, "_chart_refresh_switch", None)
        if switch is None:
            return
        if switch.isChecked():
            self._on_resume_chart_refresh()
            return
        self._clear_history_overlay()
        self._set_chart_refresh_paused(True)
        self._status_bar.showMessage("图表已暂停实时更新")
    ...
    def _on_fetch_data_clicked(self) -> None:
        ...
        compare_symbol = new_symbol
        source_kind = self._current_data_source_kind()
        if source_kind == "eastmoney_futures":
            from pa_agent.data.eastmoney_futures_source import normalize_futures_symbol

            compare_symbol = normalize_futures_symbol(new_symbol)
        elif source_kind == "futu":
            from pa_agent.data.futu_source import normalize_futu_symbol

            compare_symbol = normalize_futu_symbol(
                new_symbol, exchange=self._tv_exchange_text()
            )
        cur_symbol = str(getattr(data_source, "_symbol", "") or "").strip()
        cur_tf = str(getattr(data_source, "_timeframe", "") or "").strip()
        if new_symbol and (compare_symbol != cur_symbol or new_tf != cur_tf):
    ...
    def _ensure_refresh_loop_running(self) -> None:
        ...
        Called when the user enables 「持续跟踪分析」 or 「最新K线收盘后再分析」
    ...
    def _bars_sufficient_for_analysis(self, bars: list[Any], bar_count: int) -> bool:
        """True when at least one closed K-line can be analyzed.

        The configured count is a target rather than a prerequisite: newly
        listed instruments may legitimately have fewer historical K-lines.
        """
        return self._effective_analysis_bar_count(bars, bar_count) >= 1

    def _effective_analysis_bar_count(self, bars: list[Any], bar_count: int) -> int:
        """Return the usable closed-bar count, capped by the configured target."""
        ...
        return min(max(0, int(bar_count)), available_closed)
    ...
    def _update_refresh_elapsed(self) -> None:
        ...
        if elapsed > 10:
            label.setStyleSheet(f"color: {T.DANGER}; font-size: 11px;")
        else:
    ...
    def _on_chart_dragging_changed(self, dragging: bool) -> None:
        """Defer live chart work until panning finishes, then apply one latest tick."""
        self._chart_dragging = dragging
        if dragging:
            return
        bars = self._deferred_chart_bars
        self._deferred_chart_bars = None
        if bars:
    ...
    def _on_refresh_frame_ready(self, bars: Any) -> None:
        ...
        if self._chart_dragging:
            # Keep exactly one latest snapshot while panning.  Deliberately do
            # not build a KlineFrame here: that also skips EMA/ATR calculation,
            # main/sub-chart rendering, quote-strip updates, and live analysis
            # scheduling until the drag is released.
            self._deferred_chart_bars = list(bars) if bars else None
            return

        if bars:
            self._last_frame_ready_bars = list(bars)
            self._update_market_summary(bars)
    ...
    def _on_symbol_or_tf_changed(self, new_symbol: str, new_tf: str) -> None:
        ...
        try:
            ...
            if hasattr(self, "_chart_widget"):
                self._chart_widget.reset()
                self._chart_widget.request_fit_on_next_render()
            summary = getattr(self, "_market_summary_strip", None)
            ...
            # Check for a prior analysis record.  This changes the submit
            # button to "增量分析"; automatic waiting/submission belongs only
            # to the explicitly enabled continuous-tracking feature.
            self._refresh_history_ui()
            self._check_auto_incremental(new_symbol, new_tf)
            self._update_submit_button_state()

    def _check_auto_incremental(self, symbol: str, timeframe: str) -> None:
        """Refresh the submit mode after a symbol/timeframe switch."""
        self._auto_incremental_pending = False
        ...
        try:
            ...
            # A record alone is not enough: the current K-lines must overlap
            # the historical snapshot and remain inside the configured range.
            self._refresh_incremental_label()
            if self._incremental_available:
                self._status_bar.showMessage(
                    f"找到可对齐的历史记录，下次提交将使用增量分析（{symbol} {timeframe}）"
                )
            else:
                self._status_bar.showMessage(
                    f"找到历史记录，但当前 K 线无法增量对齐，已切换为提交分析（{symbol} {timeframe}）"
                )
        except Exception as exc:  # noqa: BLE001
    ...
    def _forming_bar_seconds_remaining(self) -> int | None:
        ...
        if ts is None or not tf:
            return None
        symbol = self._pending_submit_symbol if self._pending_submit_after_close else (
            self._symbol_combo.currentText().strip()
        )
        ...
        return seconds_until_bar_closes(
            ...
            now_ms=self._reference_now_ms(),
            symbol=symbol,
            timestamp_is_close=bool(getattr(source, "kline_timestamp_is_close", False)),
        )
        ...
        if lbl is not None:
            if not show:
                ...
                if secs is None:
                    ...
                    lbl.setStyleSheet(f"color: {T.ACCENT}; font-size: 11px;")
    ...
    def _arm_wait_for_bar_close(
        ...
        *,
        ...
        if forming_ts is None:
            next_session_open = next_ashare_session_open_ms(
                timeframe,
                symbol=symbol,
                now_ms=self._reference_now_ms(),
            )
            if next_session_open is not None:
                forming_ts = next_session_open + (
                    (timeframe_to_seconds(timeframe) or 0) * 1000
        ...
        try:
            ...
            timestamp_label = "收盘" if timestamp_is_close else "开盘"
            ts_hint = f"{timestamp_label} {dt}"
        except (OSError, OverflowError, ValueError):
            ts_hint = f"ts={forming_ts}"

        submit_hint = "提交增量分析" if force_incremental else "提交分析"
        if next_session_open is not None:
            close_seconds = 0 if timestamp_is_close else timeframe_to_seconds(timeframe) or 0
            close_dt = datetime.fromtimestamp(
                (forming_ts + close_seconds * 1000) / 1000
            ).strftime("%H:%M")
            self._status_bar.showMessage(
                f"A股休市中，等待 {close_dt} 首根K线收盘后自动{submit_hint}"
            )
    ...
    def _on_keep_analysis_checkbox_changed(self, _state: int) -> None:
        ...
        self.workspace_continuous_tracking_changed.emit(enabled)
    ...
    def _on_submit_analysis(self) -> None:
        """Submit analysis, using incremental mode only after a fresh match."""
    ...
    def _begin_submit_analysis(self, *, force_incremental: bool) -> None:
        ...
        if data_source is not None:
            ...
            if source_kind == "eastmoney_futures":
                ...
                # Futu stores subscription symbols in MARKET.CODE form.  Compare
                # the UI input in that same form, otherwise a bare code such as
                # 159732 looks like a new symbol on every submit and causes a
                # silent re-subscribe instead of starting the analysis.
                from pa_agent.data.futu_source import normalize_futu_symbol

                new_symbol = normalize_futu_symbol(
                    new_symbol_raw, exchange=self._tv_exchange_text()
                )
            else:
    ...
    def _start_analysis_with_bars(
        ...
        *,
        ...
        effective_bar_count = self._effective_analysis_bar_count(snapshot_bars, bar_count)
        if effective_bar_count < 1:
            self._status_bar.showMessage("数据不足，无法构建分析快照")
            return
        ...
        self._analysis_in_progress = True
        self._managed_order_opportunity = False
        self.order_opportunity_changed.emit(False)
        self.analysis_state_changed.emit(True)
        self._last_analysis_had_error = False
        self._update_submit_button_state()
        status = "准备分析…（构建快照）"
        if effective_bar_count < 40:
            status = "样本量低于40根，可能影响分析结果准确性；" + status
    ...
    def _incremental_unavailable_reason(
        ...
        """Explain why incremental analysis cannot start."""
        try:
            from pa_agent.records.analysis_history import (
                compute_incremental_bar_delta,
                find_latest_successful_record,
        ...
        *decision* is the full stage2 JSON dict (``{"decision": {...},
        ...
        if decision:
            ...
            chart_decision = enrich_decision_for_chart_overlay(
                ...
                allow_trade_csv_fallback=not getattr(self, "_history_overlay_active", False),
            ...
            self.order_opportunity_changed.emit(self._managed_order_opportunity)
            if getattr(self, "_history_overlay_active", False):
                pass
            elif self._maybe_alert_order_opportunity(inner):
    ...
    def _on_record_ready_impl(self, record: Any) -> None:
        ...
        self._archive_completed_history_record(record)
        if s2_full:
            from pa_agent.gui.stage2_payload import prepare_stage2_for_ui
    ...
    def _maybe_alert_order_opportunity(self, decision_inner: dict) -> bool:
        ...
        # The workspace listens before the dialog is shown, so an analysis
        # that was running in the background becomes the active stock tab.
        self.order_opportunity_detected.emit(dict(decision_inner))
        play_order_alert_sound()
        sidebar = getattr(self, "_ai_sidebar", None)
        if sidebar is not None:
    ...
    def closeEvent(self, event: QCloseEvent | None) -> None:
        ...
        try:
            ...
            if second_order is not None:
                second_order.shutdown(timeout_ms=_WORKER_JOIN_TIMEOUT_MS)
            self._cancel_analysis_worker(join_ms=_WORKER_JOIN_TIMEOUT_MS)
            self._cancel_snapshot_fetch_worker()
            self._stop_refresh_loop()
            self._disconnect_data_source(getattr(self._ctx, "data_source", None))
    ...
    def _apply_default_indicator_panel_height(self) -> None:
        """Set the initial lower-plot height once without overriding user resizing."""
        if self._indicator_panel_default_height_applied:
            return
        splitter = self._chart_indicator_splitter
        available_height = splitter.height()
        if available_height <= 0:
            QTimer.singleShot(0, self._apply_default_indicator_panel_height)
            return
        saved_ratio = self._saved_chart_indicator_split_ratio()
        if saved_ratio is not None:
            first = int(available_height * saved_ratio)
            second = available_height - first - splitter.handleWidth()
            splitter.setSizes([max(1, first), max(1, second)])
        else:
            panel_height = min(
                int(self._indicator_panel_default_height_px),
                max(self._indicator_panel.minimumHeight(), available_height - 1),
            )
            splitter.setSizes([max(1, available_height - panel_height), panel_height])
        self._indicator_panel_default_height_applied = True

    def _apply_saved_workbench_split_ratio(self) -> None:
        if self._workbench_split_ratio_applied:
            return
        splitter = self._workbench
        available_width = splitter.width()
        ratio = self._saved_split_ratio("workbench")
        if available_width <= 0 or ratio is None:
            if available_width > 0:
                self._workbench_split_ratio_applied = True
            return
        first = int(available_width * ratio)
        second = available_width - first - splitter.handleWidth()
        splitter.setSizes([max(1, first), max(1, second)])
        self._workbench_split_ratio_applied = True

    def _persist_workbench_split_ratio(self) -> None:
        sizes = self._workbench.sizes()
        total = sum(sizes)
        if total > 0:
            self._persist_split_ratio("workbench", sizes[0] / total)

    def _saved_chart_indicator_split_ratio(self) -> float | None:
        return self._saved_split_ratio("chart_indicator")

    def _saved_split_ratio(self, key: str) -> float | None:
        settings = getattr(self._ctx, "settings", None)
        general = getattr(settings, "general", None)
        ...
        try:
            ...
            return _SESSION_SPLIT_HOTZONE_RATIOS.get(key)
        ratio = min(0.95, max(0.05, ratio))
        _SESSION_SPLIT_HOTZONE_RATIOS[key] = ratio
        return ratio

    def _persist_chart_indicator_split_ratio(self, ratio: float) -> None:
        self._persist_split_ratio("chart_indicator", ratio)

    def _persist_split_ratio(self, key: str, ratio: float) -> None:
        settings = getattr(self._ctx, "settings", None)
        general = getattr(settings, "general", None)
        if general is None:
            return
        ...
        ratio = min(0.95, max(0.05, float(ratio)))
        ratios[key] = ratio
        _SESSION_SPLIT_HOTZONE_RATIOS[key] = ratio
        try:
            from pa_agent.config.paths import SETTINGS_JSON_PATH
            ...
            persisted_settings.general.split_hotzone_ratios[key] = ratio
            save_settings(persisted_settings, SETTINGS_JSON_PATH)
        except Exception:
            logger.debug("Unable to persist chart split ratio", exc_info=True)

    def _suspend_initial_embedded_scrollbars(self) -> None:
        """Hide transient scrollbars until the embedded page has final geometry."""
        self._initial_scrollbar_policies: list[
            tuple[QAbstractScrollArea, Qt.ScrollBarPolicy, Qt.ScrollBarPolicy]
        ] = []
        for area in self.findChildren(QAbstractScrollArea):
            self._initial_scrollbar_policies.append(
                (
                    area,
    ...
    def _restore_initial_embedded_scrollbars(self) -> None:
        policies = getattr(self, "_initial_scrollbar_policies", ())
        self._initial_scrollbar_policies = []
        for area, horizontal_policy, vertical_policy in policies:
            try:
                area.setHorizontalScrollBarPolicy(horizontal_policy)
                area.setVerticalScrollBarPolicy(vertical_policy)
            except RuntimeError:
                continue
    ...
    def showEvent(self, event: QShowEvent | None) -> None:
        ...
        self._apply_saved_workbench_split_ratio()
        # The splitter has a real geometry by ShowEvent; apply its initial size
        # before the first paint so its handle does not visibly jump.
        self._apply_default_indicator_panel_height()
        if self._embedded_mode:
            # Give the tab page one complete layout/paint interval before
            # restoring AsNeeded policies. This prevents a scrollbar computed
            # from the pre-layout size from flashing on screen.
            QTimer.singleShot(80, self._restore_initial_embedded_scrollbars)
            return
    ...
    def _open_ai_model_settings_dialog(self, *, focus_api_key: bool = False) -> None:
        ...
        if dlg.exec():
            self.sync_ai_model_settings(settings)
            second_order = getattr(self, "_second_order_workspace", None)
            if second_order is not None:
                second_order.refresh_settings()
            self.resubmit_after_ai_settings_saved()

    def sync_ai_model_settings(self, source_settings: Any) -> None:
        """Apply provider settings to this terminal without changing its feed."""
        ...
        # Stop an in-flight request before replacing the client object. The
        # following resubmit call performs the same cleanup defensively.
        ...
        settings = getattr(self._ctx, "settings", None)
        if settings is None:
            settings = source_settings.model_copy(deep=True)
            self._ctx.settings = settings
        else:
            settings.provider = source_settings.provider.model_copy(deep=True)
            settings.primary_provider_enabled = bool(
                getattr(source_settings, "primary_provider_enabled", True)
            )
            settings.retry_primary_each_analysis = bool(
                getattr(source_settings, "retry_primary_each_analysis", False)
            )
            # Saving the dialog is the explicit recovery action for a route
            # disabled by an earlier API failure in this terminal.
            settings.primary_provider_runtime_disabled = False
            backup = getattr(source_settings, "backup_provider", None)
            settings.backup_provider = (
                backup.model_copy(deep=True) if backup is not None else None
            )
            settings.backup_provider_enabled = bool(
                getattr(source_settings, "backup_provider_enabled", False)
            )

        # Recreate the client so provider aliases (for example Cursor SDK)
        ...
        sidebar = getattr(self, "_ai_sidebar", None)
        if sidebar is not None:
            sidebar.bind_settings(settings)
        update_api_key(key)

        session = getattr(self, "_free_chat_session", None)
        if session is not None:
            session._client = self._ctx.client
            session._settings = settings

        self._update_ai_mode_label()
        self._refresh_api_key_ui_state()

    def resubmit_after_ai_settings_saved(self) -> None:
        """Cancel stale work and submit one analysis with the new provider."""
        if self._demo_mode:
            return

        # A settings change must invalidate prep, snapshot, and API workers so
        # a late callback cannot overwrite the result from the new route.
        if getattr(self, "_pending_submit_after_close", False):
            self._clear_pending_bar_close_wait()
        self._cancel_snapshot_fetch_worker()
        self._cancel_analysis_worker(join_ms=_WORKER_JOIN_TIMEOUT_MS)
        was_in_progress = self._analysis_in_progress
        self._analysis_in_progress = False
        if was_in_progress:
            self.analysis_state_changed.emit(False)
        self._set_chart_refresh_paused(False)
        ...
        # The workspace dashboard has no active analysis target. Standalone
        # terminals and the currently selected workspace terminal do.
        if not hasattr(self, "_symbol_combo") or not hasattr(self, "_tf_combo"):
            return
        self._begin_submit_analysis(force_incremental=False)
    ...
    def start_workspace_analysis(self) -> bool:
        """Start this terminal from the watchlist workspace.

        The normal submit path is intentionally reused: it fetches a snapshot
        when needed and lets :class:`AnalysisPrepWorker` choose incremental
        analysis only when a compatible history record exists.
        """
        if self.workspace_analysis_in_progress():
            return False
        self._ensure_refresh_loop_running()
        self._begin_submit_analysis(force_incremental=False)
        return True
    ...
    def set_workspace_continuous_tracking(self, enabled: bool) -> None:
        """Synchronize the workspace pool switch with the original checkbox."""
        checkbox = getattr(self, "_keep_analysis_checkbox", None)
        if checkbox is not None and checkbox.isChecked() != enabled:
            checkbox.setChecked(enabled)

    def sync_workspace_notification_settings(self, source_settings: Any) -> None:
        """Apply workspace notification settings without replacing this stock's feed."""
        settings = getattr(self._ctx, "settings", None)
        if settings is None or source_settings is None:
            return
        try:
            settings.feishu = source_settings.feishu.model_copy(deep=True)
            settings.pushplus = source_settings.pushplus.model_copy(deep=True)
            settings.general.alert_on_order_opportunity = (
                source_settings.general.alert_on_order_opportunity
            )
            settings.general.decision_confidence_threshold = (
                source_settings.general.decision_confidence_threshold
            )
        except Exception as exc:  # noqa: BLE001
            logger.debug("Failed to sync workspace notification settings: %s", exc)

    def workspace_analysis_in_progress(self) -> bool:
        return bool(
            self._analysis_in_progress
            or getattr(self, "_snapshot_fetch_worker", None) is not None
            or self._prep_worker is not None
        )

    def workspace_continuous_tracking_enabled(self) -> bool:
        checkbox = getattr(self, "_keep_analysis_checkbox", None)
        return bool(checkbox is not None and checkbox.isChecked())

    def workspace_order_opportunity(self) -> bool:
        return bool(self._managed_order_opportunity)

    def workspace_progress_text(self) -> str:
        return self._status_bar.currentMessage() or "等待分析"
    ...
    def _submit_block_reason(self) -> str | None:
        ...
        if self._analysis_in_progress:
            return "分析进行中"
        if getattr(self, "_wait_close_checkbox", None) is not None and (
            self._wait_close_checkbox.isChecked()
        ):
            return "等待最新K线收盘"
        if self._pending_submit_after_close:
            return "等待最新K线收盘"
    ...
    def _refresh_incremental_label(self) -> None:
        """Show incremental mode only when current K-lines match history."""
        if not hasattr(self, "_submit_btn"):
            return
        ...
        try:
            ...
            if frame is None:
                raise ValueError("unable to build analysis frame")
            previous, _new_count, _detail = self._find_incremental_base_record(
                frame,
                symbol,
                timeframe,
            )
    ...
    def _update_market_summary(self, bars: Any) -> None:
        """Refresh the compact quote row above the K-line chart.

        A source may supply a richer live quote (Futu does); otherwise the
        newest K-line remains a useful, source-agnostic fallback.
        """
        strip = getattr(self, "_market_summary_strip", None)
        if strip is None or not bars:
            return
        head = bars[0]
        quote: dict[str, Any] = {}
        data_source = getattr(self._ctx, "data_source", None)
        getter = getattr(data_source, "latest_market_summary", None)
        if callable(getter):
            try:
                raw = getter()
                ...
                logger.debug("Market summary unavailable", exc_info=True)

        strip.set_summary(
            latest_price=quote.get("last_price", getattr(head, "close", None)),
            open_price=quote.get("open_price", getattr(head, "open", None)),
            change_rate=quote.get("change_rate", getattr(head, "pct_chg", None)),
            high_price=quote.get("high_price", getattr(head, "high", None)),
            low_price=quote.get("low_price", getattr(head, "low", None)),
        )
    ...
    def _build_chart_frame_from_bars(
        ...
        *,
        ...
        if bar_count is None:
            ...
            available_closed = len(bars_raw) - int(
                has_forming_bar_at_head(
                    bars_raw,
                    timeframe,
                    symbol=symbol,
            ...
            n = min(self._analysis_bar_count(), available_closed)
        else:
            n = bar_count
        if n < 1:
            return None
