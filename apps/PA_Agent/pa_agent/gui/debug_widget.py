...
class DebugWidget(QWidget):
    ...
    def _build_button_row(self) -> QHBoxLayout:
        ...
        btn_format_response = QPushButton("格式化 JSON")
        btn_format_response.setObjectName("ghostButton")
        btn_format_response.clicked.connect(self._format_current_response_json)
        row.addWidget(btn_format_response)
    ...
    def _format_current_response_json(self) -> None:
        row = self._current_row()
        if row < 0 or row >= len(self._turns):
            return
        raw = self._turns[row].get("raw_response", {})
        if not isinstance(raw, dict):
            return
        self._response_edit.setPlainText(
            self._mask(json.dumps(raw, ensure_ascii=False, indent=2, sort_keys=True))
        )
