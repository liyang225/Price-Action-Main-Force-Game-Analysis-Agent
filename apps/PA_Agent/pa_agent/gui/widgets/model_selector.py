...
class _ModelOption(QWidget):
    ...
    def __init__(
        ...
        self._dot.setStyleSheet("background-color: #646E7A; border-radius: 3px;")

        self._name = QLabel(model_name)
        self._name.setStyleSheet("color: #E8ECF1; font-size: 12px;")

        self._meta = QLabel(meta)
        self._meta.setStyleSheet(
            "color: #646E7A; font-size: 11px; font-family: monospace;"
        )
    ...
    def _refresh_style(self) -> None:
        if self._selected:
            self.setStyleSheet(
                "_ModelOption {"
                " background-color: rgba(74,126,187,0.12);"
                " border-radius: 6px;"
                "}"
                "_ModelOption:hover {"
                " background-color: rgba(74,126,187,0.18);"
                "}"
            )
            self._dot.setStyleSheet(
                "background-color: #4A7EBB; border-radius: 3px;"
            )
        else:
            self.setStyleSheet(
                "_ModelOption {"
            ...
            self._dot.setStyleSheet(
                "background-color: #646E7A; border-radius: 3px;"
            )
...
class ModelSelector(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._button.setStyleSheet(
            ...
            " height: 24px;"
            " padding: 0 10px;"
            " border: 1px solid rgba(74,126,187,0.35);"
            " border-radius: 999px;"
            " background-color: rgba(74,126,187,0.10);"
            " color: #7FA6CF;"
            ...
            "}"
            "QPushButton:pressed {"
            " background-color: rgba(74,126,187,0.25);"
            "}"
        )
