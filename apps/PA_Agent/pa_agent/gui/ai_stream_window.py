...
_RED_PCT = 95.0
_STYLE_NORMAL = ""
_STYLE_YELLOW = "QProgressBar#tokenProgress::chunk { background-color: #c0913c; }"
_STYLE_RED = "QProgressBar#tokenProgress::chunk { background-color: #FF4757; }"
...
class AIStreamPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._prompt_library = PromptLibraryStore()
    ...
    def _setup_ui(self) -> None:
        ...
        self._reasoning_toggle.setText("思考过程 (0 字) · 收起")
        self._reasoning_toggle.setToolButtonStyle(
            Qt.ToolButtonStyle.ToolButtonTextBesideIcon
        )
        self._reasoning_toggle.setArrowType(Qt.ArrowType.DownArrow)
        self._reasoning_toggle.setStyleSheet(
            "QToolButton { color: #9AA5B1; font: 13px 'JetBrains Mono', 'Cascadia Mono'; }"
            "QToolButton:hover { color: #E8ECF1; }"
        )
    ...
    def _apply_stream_font(self) -> None:
        ...
        if self._settings is not None:
            point_size = int(getattr(self._settings.general, "stream_pane_font_pt", 11) or 11) + 1
    ...
    def _build_input_area(self) -> QWidget:
        ...
        self._prompt_toggle = QToolButton()
        self._prompt_toggle.setText("提示词")
        self._prompt_toggle.setCheckable(True)
        self._prompt_toggle.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextBesideIcon)
        self._prompt_toggle.toggled.connect(self._toggle_prompt_library)
        prompt_row.addWidget(self._prompt_toggle)
        self._quick_prompt_buttons: list[QPushButton] = []
        for _ in range(3):
            button = QPushButton()
            button.setObjectName("quickPromptButton")
            button.setVisible(False)
            button.setMaximumWidth(168)
            button.clicked.connect(lambda _checked=False, target=button: self._insert_quick_prompt(target))
            prompt_row.addWidget(button)
            self._quick_prompt_buttons.append(button)
        prompt_row.addStretch()
        layout.addLayout(prompt_row)

        self._prompt_list = QListWidget()
        self._prompt_list.setMaximumHeight(140)
        self._prompt_list.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self._prompt_list.itemClicked.connect(self._insert_prompt)
        self._prompt_list.customContextMenuRequested.connect(self._show_prompt_context_menu)
        self._prompt_list_panel = QWidget()
        prompt_list_layout = QVBoxLayout(self._prompt_list_panel)
        ...
        self._add_prompt_button = QPushButton("+ 添加提示词")
        self._add_prompt_button.setObjectName("ghostButton")
        self._add_prompt_button.clicked.connect(self._add_prompt)
        prompt_list_actions.addWidget(self._add_prompt_button)
        prompt_list_actions.addStretch()
        prompt_list_layout.addLayout(prompt_list_actions)
        prompt_list_layout.addWidget(self._prompt_list)
        self._prompt_list_panel.setVisible(False)
        layout.addWidget(self._prompt_list_panel)
    ...
    def _toggle_prompt_library(self, visible: bool) -> None:
        self._prompt_toggle.setText("提示词 ▾" if visible else "提示词 ▸")
        self._prompt_list_panel.setVisible(visible)
        self._add_prompt_button.setVisible(visible)
        if visible:
            self._refresh_prompt_list()

    def _refresh_prompt_list(self) -> None:
        self._prompt_library = type(self._prompt_library)()
        self._prompt_list.clear()
        snippets = list(self._prompt_library.items)
        snippets.sort(key=lambda snippet: -self._prompt_usage.get(snippet.id, 0))
        for button, snippet in zip(self._quick_prompt_buttons, snippets[:3], strict=False):
            button.setText(snippet.name)
            button.setToolTip(snippet.text)
            button.setProperty("promptId", snippet.id)
            button.setVisible(True)
        for button in self._quick_prompt_buttons[len(snippets[:3]) :]:
            button.setVisible(False)
    ...
    def _insert_quick_prompt(self, button: QPushButton) -> None:
        item_id = button.property("promptId")
        snippet = self._prompt_library.get(item_id) if isinstance(item_id, str) else None
        if snippet is None:
            return
        cursor = self._input_edit.cursorPosition()
        current = self._input_edit.text()
        self._input_edit.setText(current[:cursor] + snippet.text + current[cursor:])
        self._input_edit.setCursorPosition(cursor + len(snippet.text))
        self._input_edit.setFocus()
        self._prompt_usage[snippet.id] = self._prompt_usage.get(snippet.id, 0) + 1
        self._refresh_prompt_list()

    def _insert_prompt(self, list_item: QListWidgetItem) -> None:
    ...
    def _show_prompt_context_menu(self, position) -> None:
        list_item = self._prompt_list.itemAt(position)
        ...
        edit_action.triggered.connect(lambda: self._edit_prompt(item_id))
        delete_action = menu.addAction("删除提示词")
        delete_action.triggered.connect(lambda: self._delete_prompt(item_id))
        menu.exec(self._prompt_list.viewport().mapToGlobal(position))

    def _add_prompt(self) -> None:
        self._edit_prompt()

    def _edit_prompt(self, item_id: str | None = None) -> None:
        snippet = self._prompt_library.get(item_id) if item_id is not None else None
        dialog = QDialog(self)
        ...
        try:
            if snippet is None:
                self._prompt_library.add(name=name_edit.text(), text=text_edit.toPlainText())
            else:
                self._prompt_library.update(
    ...
    def _delete_prompt(self, item_id: str) -> None:
        snippet = self._prompt_library.get(item_id)
        if snippet is None:
            return
        ...
        if answer == QMessageBox.StandardButton.Yes:
            self._prompt_library.remove(item_id)
            self._refresh_prompt_list()
    ...
    def _update_stats(self) -> None:
        ...
        expanded = self._reasoning_toggle.isChecked()
        suffix = "收起" if expanded else "点击展开"
        self._reasoning_toggle.setText(
            f"思考过程 ({self._reasoning_chars:,} 字) · {suffix}"
        )

    def _toggle_reasoning(self, visible: bool) -> None:
        self._reasoning_edit.setVisible(visible)
