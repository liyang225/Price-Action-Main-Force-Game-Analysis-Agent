...
_SUPPORTED_TIMEFRAMES: tuple[str, ...] = ("1h", "2h", "4h", "1d")
...
def _resample_rows_to_2h(rows_asc: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Combine 60-minute rows into A-share morning/afternoon sessions."""
    buckets: list[dict[str, Any]] = []
    current_day = None
    day_rows: list[dict[str, Any]] = []

    def flush() -> None:
        for index in range(0, len(day_rows), 2):
            chunk = day_rows[index : index + 2]
            if chunk:
                buckets.append(_merge_ohlcv(chunk))
    ...
    for row in rows_asc:
        ...
        if isinstance(value, str):
            ...
            day = datetime.fromtimestamp(float(value) / 1000, tz=_CN_TZ).date()
...
class AkShareSource(DataSource):
    ...
    def _fetch_history(self, symbol: str, timeframe: str, n: int) -> list[dict[str, Any]]:
        try:
            ...
            if timeframe == "2h":
                rows_60 = self._fetch_minute_ak(symbol, "60", n * 2 + 8)
                return _resample_rows_to_2h(rows_60)[-n:]
            if timeframe == "4h":
    ...
    def _fetch_history_baostock(
        ...
        freq = {"1d": "d", "1h": "60", "2h": "60", "4h": "60"}.get(timeframe, "d")
        end = _cn_now().strftime("%Y-%m-%d")
        start = (_cn_now() - timedelta(days=max(n * 2, 400))).strftime("%Y-%m-%d")
        ...
        if freq != "d":
            ...
            if timeframe == "2h":
                return _resample_rows_to_2h(rows)[-n:]
            if timeframe == "4h":
