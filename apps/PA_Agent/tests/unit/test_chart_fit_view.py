...
class TestChartFitView:
    ...
    def test_vertical_range_preserves_requested_candle_scale(self, chart_widget):
        from pa_agent.gui.chart_widget import (
            _CANDLE_VERTICAL_SCALE,
            _Y_PADDING_RATIO,
            _Y_TOP_EXTRA_RATIO,
        )

        frame = _sample_frame(n=5)
        _, y_range = chart_widget._view_ranges_for_frame(frame)
        price_min = min(bar.low for bar in frame.bars)
        price_max = max(bar.high for bar in frame.bars)
        price_span = price_max - price_min
        unscaled_view_span = price_span * (
            1.0 + 2.0 * _Y_PADDING_RATIO + _Y_TOP_EXTRA_RATIO
        )

        assert (y_range[1] - y_range[0]) == pytest.approx(
            unscaled_view_span / _CANDLE_VERTICAL_SCALE
        )
...
class TestResizableAxisItem:
    ...
    def test_chart_press_immediately_pauses_live_updates(self, chart_widget, qtbot):
        """A press in the ViewBox pauses live work before the pointer moves."""
        from PyQt6.QtCore import QPointF
        from PyQt6.QtGui import QMouseEvent
        from PyQt6.QtCore import QEvent, Qt
        from PyQt6.QtWidgets import QApplication
        ...
        assert chart_widget._axis_resizing is False
        assert chart_widget._chart_dragging is True
        ...
        assert chart_widget._chart_dragging is False
